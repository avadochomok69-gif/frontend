...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [cart, setCart] = useState<CartItem[]>(() => {
    const saved = localStorage.getItem('chomok_cart');
    if (!saved) return [];
    try {
  ...
  useEffect(() => {
    localStorage.setItem('chomok_cart', JSON.stringify(cart));
  }, [cart]);
  ...
  const updateItemVariation = (
    productId: string, oldVariationId: string | undefined, newVariationId: string | undefined,
    newVariationName: string, newPrice: number, newImage: string, newName: string
  ) => {
    setCart(prev => {
