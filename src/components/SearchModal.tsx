...
const SearchModal = ({ isOpen, onClose }: Props) => {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          <motion.div className="search-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} />
          <motion.div
          ...
          >
            <div className="search-input-row">
              ...
              <input
                ...
                placeholder="Search for clothes, fashion, accessories..."
                value={query}
              ...
              <button className="search-close-btn" onClick={onClose} type="button" aria-label="Close">
                <X size={20} />
              </button>
            </div>
            <div className="search-results-area">
              {!query && !isLoading && (
                <div className="search-placeholder">
                  ...
                  <p>Search for your favourite fashion</p>
                  <span>Try "shirt", "dress", "accessories"</span>
                </div>
              )}
              {isLoading && (
              ...
              {results.length > 0 && (
                <div className="search-results-list">
                  ...
                  {results.map(product => (
                    <div key={product.id} className="search-result-item" onClick={() => handleProductClick(product.id)} role="button" tabIndex={0}>
                      <img src={product.mainImage} alt={product.name} className="search-result-img" />
                      <div className="search-result-info">
                        {product.category?.name && <span className="search-result-category">{product.category.name}</span>}
                        <h4 className="search-result-name">{product.name}</h4>
                        <span className="search-result-price">{formatCurrency(product.price)}</span>
                      </div>
                      <button className="search-result-cart-btn" onClick={e => handleAddToCart(e, product)} aria-label="Add to cart" type="button">
                        <ShoppingBag size={16} />
                      </button>
                    </div>
                  ))}
