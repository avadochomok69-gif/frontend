...
const CheckoutItem: React.FC<CheckoutItemProps> = ({ item }) => {
  ...
  useEffect(() => {
    ...
    groups.forEach(g => { if (initialOpts[g] === undefined) initialOpts[g] = null; });
  ...
  const handleOptionChange = (groupName: string, optionId: string) => {
    ...
    const newVariationName = Object.values(newOptions).filter(Boolean).map((o) => `${o!.name}: ${o!.value}`).join(', ');
    ...
    <div className="summary-item">
      ...
      <div className="summary-details">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.5rem' }}>
          <button type="button" style={{ padding: '6px 10px', background: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)', borderRadius: '6px', cursor: 'pointer', color: 'var(--color-orange)' }} onClick={() => updateQuantity(item.productId, item.quantity - 1, item.variationId)}>−</button>
          <input
            aria-label="Quantity"
            type="number"
            min={1}
            ...
            style={{ width: 60, padding: '6px 8px', textAlign: 'center', border: '1px solid var(--color-border)', borderRadius: '6px', fontFamily: 'var(--font-sans)' }}
          />
          <button type="button" style={{ padding: '6px 10px', background: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)', borderRadius: '6px', cursor: 'pointer', color: 'var(--color-orange)' }} onClick={() => updateQuantity(item.productId, item.quantity + 1, item.variationId)}>+</button>
        </div>
      </div>
